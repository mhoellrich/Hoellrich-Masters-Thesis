...
Transects0<-Transects
Transects0$Site[Transects0$Site == "Amt1"] <- "AMT"
Transects0$Site[Transects0$Site == "Amt2"] <- "AMT"
Transects0$Site[Transects0$Site == "Jer1"] <- "JER"
Transects0$Site[Transects0$Site == "Jer2"] <- "JER"
Transects0$Site[Transects0$Site == "Vol1"] <- "VOL"
Transects0$Site[Transects0$Site == "Vol2"] <- "VOL"
Transects0$Site[Transects0$Site == "Whs1"] <- "WHS"
Transects0$Site[Transects0$Site == "Whs2"] <- "WHS"

TopCov <- function(x){  if(x == "") result <- "Open"
else result <- "Plant"
return(result)
}

LightSat<-apply(Transects0, 2, TopCov)
LightSat<-cbind(AllSite[,1:4],LightSat)

Transects0$TopCanopy<-TopCov(Transects0$TopCanopy)
CountTop<-Transects0 %>% group_by(Site,TopCanopy) %>% tally()



ATop <- Transects0[Transects0$Site == "AMT",]
sum(is.na(ATop))
str(ATop)
ATop$TopCanopy[ATop$TopCanopy == ""] <- "Open"
ATop$TopCanopy<-as.character(ATop$TopCanopy)

DivMean <- aggregate(ATop$TopCanopy, by=list(ATop$Site), FUN=count)
View(DivMean)
...
CrustCov<-ggplot(data=CrustCover,aes(x=Crust_Type,y=Percent)) + geom_bar(color=c(COLSCALE3),fill=c(COLSCALE3),stat="identity") +
  scale_x_discrete(limits = c("IC", "LAC", "DAC",  "Pelt","Clav", "Col", "M"))+
